...
class Game extends GameDB
{
    ...
    public function getTransactionsAttribute()
    {
        if ($this->transaction_ids) {
            $transactions = [];
            $transactionIds = explode(',', $this->transaction_ids);
            foreach ($transactionIds as $transactionId) {
                $transactions[] = "<a href='/nova/resources/transactions/{$transactionId}' class='no-underline font-bold dim text-primary'>{$transactionId}</a>";
            }
